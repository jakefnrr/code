const SOUNDS = {
    ...
    omg:               { file: "sounds/omg.mp3",             emoji: "🫨",  name: "omg" },
    ...
    among_us_reveal:   { file: "sounds/among-us-role-reveal.mp3", emoji: "👤", name: "among us role reveal" },
    ...
    megalovania:       { file: "sounds/megalovania.mp3",       emoji: "🎵",  name: "megalovania", group: "songs" },
    subway_surfers:    { file: "sounds/subway-surfers.mp3",    emoji: "🛹",  name: "subway surfers", group: "songs" },
    default_dance:     { file: "sounds/default-dance.mp3",     emoji: "🕺",  name: "fortnite default dance" },
    griddy:            { file: "sounds/griddy.mp3",            emoji: "💃",  name: "get griddy" },
    ...
    six_seven:         { file: "sounds/67.mp3",                emoji: "🧠", name: "6️⃣7️⃣" },
    get_out:           { file: "sounds/get-out.mp3",           emoji: "😡", name: "Get Out" },
    english_or_spanish_meme: { file: "sounds/english-or-spanish-meme.mp3", emoji: "🤨", name: "English or Spanish?" },
    messenger_notification: { file: "sounds/messenger-notification.mp3", emoji: "💬", name: "Messenger Notification" },
    nokia_ringtone:    { file: "sounds/nokia-ringtone.mp3",      emoji: "📱", name: "Nokia Ringtone" },
    ring_doorbell:     { file: "sounds/ring-doorbell.mp3",       emoji: "🚪", name: "Ring Doorbell" },
    long_brain_fart:   { file: "sounds/long-brain-fart.mp3",     emoji: "💨", name: "Long Brain Fart" },
};
...
function loadState() {
    try {
        const saved = localStorage.getItem("soundboard");
...
function saveState() {
    state.version = 1;
    try {
        localStorage.setItem("soundboard", JSON.stringify(state));
...
// ---- Embedded-audio helpers: everything works offline / file:// with no server ----
function soundBase64(key) {
    const fp = (SOUNDS[key] && SOUNDS[key].file) ? SOUNDS[key].file.split("/").pop() : (key + ".mp3");
    return (typeof SOUNDS_DATA !== "undefined" && SOUNDS_DATA) ? (SOUNDS_DATA[fp] || null) : null;
}
...
function b64ToArrayBuffer(b64) {
    return b64ToUint8(b64).buffer;
}

const soundFileCache = {};   // key -> File
const cachedBlobUrls = {};   // key -> blob URL (created once)

function getSoundFile(key) {
    if (soundFileCache[key]) return soundFileCache[key];
    const b64 = soundBase64(key);
    if (!b64) return null;
    soundFileCache[key] = new File([b64ToUint8(b64)], key + ".mp3", { type: "audio/mpeg" });
    return soundFileCache[key];
}

function getSoundBlobUrl(key) {
    if (cachedBlobUrls[key]) return cachedBlobUrls[key];
    const b64 = soundBase64(key);
    if (!b64) return null;
    cachedBlobUrls[key] = URL.createObjectURL(new Blob([b64ToUint8(b64)], { type: "audio/mpeg" }));
    return cachedBlobUrls[key];
}
...
async function decodeSound(key) {
    try {
        ...
        const b64 = soundBase64(key);
        if (!b64) throw new Error("no embedded data");
        const buf = getAudioContext().decodeAudioData(b64ToArrayBuffer(b64));
        bufferCache[key] = await buf;
    } catch (e) {
...
function getCachedAudio(key) {
    ...
    const a = new Audio(getSoundBlobUrl(key) || SOUNDS[key].file);
    a.preload = "auto";
    a.load();
    return a;
}
...
function getDuration(key) {
    return new Promise(resolve => {
        ...
        if (s) {
            ...
            const tmp = new Audio(getSoundBlobUrl(key) || s.file);
            tmp.addEventListener("loadedmetadata", () => resolve(tmp.duration));
            tmp.addEventListener("error", () => resolve(0));
        } else {
            const combo = state.customCombos.find(c => c.id === key);
            if (combo) resolve(combo.duration); else resolve(0);
        }
    });
}
...
function playCombo(combo) {
    ...
    const timers = [];
    const tracks = [];
    const totalItems = combo.items.filter(item => SOUNDS[item.sound]).length;
    let endedCount = 0;
    ...
    combo.items.forEach(item => {
        ...
        const start = () => {
            ...
            if (item.repeat) {
                ...
                const playMs = Math.max(200, (combo.duration - item.position) * 1000);
...
function renderCreateSoundGrid() {
    ...
    Object.keys(SOUNDS).forEach(key => {
        ...
        card.addEventListener("click", () => {
            state.createItems.push({ sound: key, position: 0, repeat: false });
            save();
            renderCreateItems();
...
document.getElementById("save-combo-btn").addEventListener("click", () => {
    ...
    const combo = {
        ...
        emoji,
        duration,
        items: JSON.parse(JSON.stringify(state.createItems))
    };
...
document.getElementById("test-combo-btn").addEventListener("click", () => {
    ...
    items.forEach(item => {
        ...
        const start = () => {
            ...
            if (item.repeat) {
                const repeatHandler = applyRepeatLoop(a, item);
                const playMs = Math.max(200, (duration - item.position) * 1000);
                timers.push(setTimeout(() => {
                    if (repeatHandler) repeatHandler();
                    a.pause();
                    a.currentTime = 0;
...
function renderDownloadGrid() {
    ...
    Object.keys(SOUNDS).forEach(key => {
        ...
        a.addEventListener("click", async (e) => {
            e.preventDefault();
            const filename = cleanFileName(s.name, keyPlaceholder);
            const file = getSoundFile(keyPlaceholder);
            if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
                try {
                    ...
                    return;
                } catch (er) { /* user cancelled or share failed - fall through */ }
            }
            const url = soundBlobUrl(keyPlaceholder);
            if (!url) {

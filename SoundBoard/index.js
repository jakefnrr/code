const SOUNDS = {
    ...
    stonks:            { file: "sounds/stonks.mp3",          emoji: "📈",  name: "stonks" },
    alarm:             { file: "sounds/alarm.mp3",           emoji: "🚨",  name: "alarm" },
...
function preloadAll() {
    Object.keys(SOUNDS).forEach(key => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            a.preload = "auto";
            a.load();
            preloadPools[key].push(a);
        }
    });
}

function getCachedAudio(key) {
    const pool = preloadPools[key];
    if (pool) {
        ...
        const extra = new Audio(SOUNDS[key].file);
        extra.preload = "auto";
        return extra;
    }
    ...
    return new Audio(SOUNDS[key].file);
}
...
function getDuration(key) {
    return new Promise(resolve => {
        const s = SOUNDS[key];
        if (s) {
            const a = preloadPools[key] && preloadPools[key][0];
            if (a && a.duration && !isNaN(a.duration)) return resolve(a.duration);
...
function playCombo(combo) {
    ...
    const entry = { sources, timers, tracks };
    activeAudio[combo.id].push(entry);
    updateComboCardState(combo.id);
}
...
function renderCreateTimelineBarWide() {
    ...
    state.createItems.forEach((item, idx) => {
        ...
        getDuration(item.sound).then(dur => {
            ...
            block.dataset.index = idx;
            block.style.position = "absolute";
            block.style.left = Math.min(98, ((item.position / duration) * 100)) + "%";
...
function updateNowPlayingBar() {
    ...
    Object.keys(activeAudio).forEach(key => {
        if (isComboId(key)) {
            ...
            const s = SOUNDS[key];
            if (!s) return;
            entries.push({ id: key, emoji: s.emoji, name: s.name, audios: activeAudio[key] });
        }
    });
    ...
    Object.keys(loops).forEach(key => {
        if (isComboId(key)) {
            ...
            const s = SOUNDS[key];
            if (!s) return;
            entries.push({ id: key, emoji: s.emoji, name: s.name + " 🔁", audios: [loops[key]] });
        }
    });
